    ...
    <form method="POST" action="/contracts/store" class="space-y-5">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
        <div>
          ...
          <select name="customer_id" required class="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm text-slate-800 bg-white focus:ring-2 focus:ring-sky-500 focus:border-sky-500">
            <option value="">-- Select Customer --</option>
            <?php foreach ($customers as $c): ?>
              <option value="<?= e($c['id']) ?>" <?= (($_POST['customer_id'] ?? '') == $c['id']) ? 'selected' : '' ?>><?= e($c['customer_code'] . ' — ' . $c['name']) ?></option>
            <?php endforeach; ?>
          </select>
        </div>
        <div>
          <label class="block text-sm font-medium text-slate-700 mb-1">Product <span class="text-red-500">*</span></label>
          <select name="product_id" required class="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm text-slate-800 bg-white focus:ring-2 focus:ring-sky-500 focus:border-sky-500">
        ...
        <div>
          ...
          <select name="service_type_id" required class="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm text-slate-800 bg-white focus:ring-2 focus:ring-sky-500 focus:border-sky-500">
        ...
        <div>
          ...
          <select name="branch_id" required class="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm text-slate-800 bg-white focus:ring-2 focus:ring-sky-500 focus:border-sky-500">
        ...
        <div>
          ...
          <input type="date" name="installation_date" value="<?= e($_POST['installation_date'] ?? '') ?>" required class="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm text-slate-800 bg-white focus:ring-2 focus:ring-sky-500 focus:border-sky-500">
        </div>
        <div>
          <label class="block text-sm font-medium text-slate-700 mb-1">Serial Number</label>
          <input type="text" name="serial_number" value="<?= e($_POST['serial_number'] ?? '') ?>" class="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm text-slate-800 bg-white focus:ring-2 focus:ring-sky-500 focus:border-sky-500" placeholder="e.g. SN-00123">
        </div>
        <div>
          <label class="block text-sm font-medium text-slate-700 mb-1">Assigned Technician</label>
          <select name="assigned_technician_id" class="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm text-slate-800 bg-white focus:ring-2 focus:ring-sky-500 focus:border-sky-500">
        ...
        <div>
          ...
          <select name="status" class="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm text-slate-800 bg-white focus:ring-2 focus:ring-sky-500 focus:border-sky-500">
            <?php foreach (['active'=>'Active','expired'=>'Expired','suspended'=>'Suspended','cancelled'=>'Cancelled'] as $val => $label): ?>
